...
def star_spot(star: np.array, angular_velocity=0., initial_longitude=0., time_change=0., radius=0., brightness=1.):
    ...
    if not (radius == 0 or brightness == 1):
        ...
        foreshortening = abs(np.sin(lon))

        if foreshortening > 0.01:  # avoid division by 0
            x_distance = np.abs(x_indices - spot_center_x)
            y_distance = np.abs(y_indices - spot_center_y)

            h_compression = foreshortening # horizontal copression
...
def transit(star: np.array, mask: np.array, period: float, eccentricity: float, sma: float, inclination: float,
            ...
            steps: int = 1000, angular_velocity=0, spot_longitude=0, spot_radius=0, spot_brightness=0) -> tuple:
    """
    ...
    :param steps: s
    :param angular_velocity: ω_S
    :param spot_longitude: λ_0
    :param spot_radius: r_spot
    :param spot_brightness: B
    :return: Δm(t)
    """
    ...
    if abs(np.cos(lan_rad)) < 1e-9:
        u_rad = np.pi / 2.0
    elif abs(np.sin(lan_rad)) < 1e-9:
        u_rad = 0.0
    else:
    ...
    # Convert eccentric anomaly (E) to mean anomaly and then to time (t).
    ...
    if sma > 0:
        ...
        half_search_window = 4.0 * estimated_duration
    else:
...
def show_model(model: np.ndarray, title='Optical Depth Distribution Map', x_label='x', y_label='y', colorbar_label='Optical Depth', vmin=None, vmax=None) -> None:
    ...
    img = plt.imshow(model, origin='lower', cmap='viridis', vmin=vmin, vmax=vmax, aspect='equal')
...
def transit_animation(star: np.array, mask: np.array, period: float, eccentricity: float, sma: float, inclination: float, longitude_of_ascending_node: float, argument_of_periapsis: float,
            steps: int = 1000, angular_velocity=0, spot_longitude=0, spot_radius=0, spot_brightness=0) -> list:
    """
    ...
    (This function was created by Gemini 3)
...
def transit_simple(star: np.array, mask: np.array, impact_parameter: float, 
                  velocity: float, steps: int = 500) -> tuple:
    """
    Simplified transit model using impact parameter instead of full orbital geometry.
    
    This function models a transit by moving the planet/mask across the star
    along a straight line at a fixed distance from the star center (impact parameter).
    Much simpler than the full orbital mechanics in transit().
    
    :param star: The 2D array of the star
    :param mask: The 2D transmission mask of the exoplanet with rings
    :param impact_parameter: Distance from star center in units of stellar radii (0 = central transit)
    :param velocity: Velocity of the planet across the stellar disk in units of stellar radii per time unit
    :param steps: Number of time steps to sample
    :return: (transit_duration, light_curve_data) where light_curve_data is [(phase, intensity), ...]
    """
    initial_intensity = np.sum(star)
    ...
    impact_px = impact_parameter * star_radius_px
    
    path_y = center_y_star + impact_px
    
    mask_radius_px = np.sqrt(h_mask ** 2 + w_mask ** 2) / 2
    total_path_length = 2 * (star_radius_px + mask_radius_px)
    
    transit_duration = total_path_length / (velocity * star_radius_px)
    
    time_points = np.linspace(0, transit_duration, steps)
    
    data_points = []
    
    for t in time_points:
        x_offset = -star_radius_px - mask_radius_px + velocity * star_radius_px * t
        planet_x = center_x_star + x_offset
        planet_y = path_y
        
        light_blocked = 0.0
        
        tl_x = int(round(planet_x - w_mask / 2))
        tl_y = int(round(planet_y - h_mask / 2))
        ...
        if star_y_end > star_y_start and star_x_end > star_x_start:
            ...
            mask_slice = mask[mask_y_start:mask_y_end, mask_x_start:mask_x_end]
            show_model(mask_slice)
            if star_slice.shape == mask_slice.shape:
                light_blocked = np.sum(star_slice - star_slice * np.exp(-mask_slice))
        
        current_intensity = initial_intensity - light_blocked
        
        phase = t / transit_duration
        data_points.append((phase, current_intensity))
    
    return transit_duration, light_curve(data_points)


if __name__ == '__main__':
        star = quadratic_star_model([1001, 1001], [0.9, 0.9])
        planet = planet(100, 201)
        print(transit_simple(star, planet, 0, 0.1))